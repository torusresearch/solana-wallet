...
import { ACTIVITY_ACTION_TOPUP, ACTIVITY_STATUS_CANCELLED, ACTIVITY_STATUS_UNSUCCESSFUL, TransactionStatus } from "@toruslabs/base-controllers";
...
const getTxStatusColor = (status: string): string => {
  ...
  if ([TransactionStatus.finalized.toString(), TransactionStatus.confirmed.toString()].includes(status)) return "#9BE8C7";
