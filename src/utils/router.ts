...
import { getB64DecodedData, getRedirectConfig } from "./redirectflow_helpers";
...
export function hasInstanceId(route: RouteLocationNormalized) {
  return Object.prototype.hasOwnProperty.call(route.query, "instanceId");
}

export async function beforeEnterRedirect(to: RouteLocationNormalized) {
  ...
  if (!resolveRoute || !method) return { name: "404", query: to.query, hash: to.hash };
  ...
  if (shouldRedirect) return `${redirectPath}?resolveRoute=${resolveRoute}${to.hash}`;
  return true;
}

export async function restoreOrlogout() {
...
export async function beforeEachResolve(to: RouteLocationNormalized) {
  ...
  if (to.name === "404" || to.query.redirectTo) return true; // to prevent 404 redirecting to 404

  if (isRedirectFlow && (!getB64DecodedData(to.hash).method || !to.query.resolveRoute)) {
...
export const walletRoutes = [
  ...
  },
  {
    ...
    beforeEnter: async (to: RouteLocationNormalized) => {
      const result = await beforeEnterRedirect(to);
      return result;
    },
  },
  {
